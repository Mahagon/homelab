...
DNS read/write for `example.invalid`. Keep the state directory on an encrypted
...
infrastructure/opentofu/home-assistant-alexa/ask/provision-proxy-skill.ps1 `
  -VendorId <VENDOR_ID> `
  -EndpointUrl "https://homeassistant.example.invalid/api/alexa/smart_home" `
...
Token URL:         https://homeassistant.example.invalid/auth/token
...
```powershell
curl.exe --include https://homeassistant.example.invalid/api/alexa/smart_home
